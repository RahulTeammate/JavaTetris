...
public class OTest {
	...
	public void construction_and_basic_get_set_tests() {
		O piece = new O();
		int[] currentPositions = {0, 4, 0, 5, 1, 4, 1, 5};
	...
	public void rotating_piece_test() {
		O piece = new O();
		int[] orientations = {0, 4, 0, 5, 1, 4, 1, 5};
		...
		for (int rot = 1; rot < 5; rot++) {
			...
			assertArrayEquals(piece.rotatingTetBlockCoords(), orientations);
			
			//Assert that rotatingTetBlockCoords() resets the rotate.
			assertEquals(piece.getRotation(), Rotate.NONE);
			
			piece.setAllTetBlockCoords(orientations);
			assertArrayEquals(piece.getTetBlockCoords(), orientations);
		}
	}
}
